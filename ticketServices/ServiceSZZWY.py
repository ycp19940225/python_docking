#coding=UTF-8
#苏州植物园闸机系统，尚未采取定单同步对接方式。目前为：植物园闸机系统通过微景通的接口查询门票状态，正常后，再通过微景通接口核销定单（2017-03-27）
#code: SZZWY
...
def ServiceSZZWY(config, ticketBoughtList):
    ...
    for ticketBought in ticketBoughtList:
        ...
        for conf in config:
            try:
                if conf == 'viewid_' + str(ticketBought['ticket_id']):
                    # 给第三方增加门票
                    t = threading.Thread(target = addTicketToOuterSZZWY, args=(config, ticketBought,))
                    t.start()
                    ts.append(t)
                    break
...
def addTicketToOuterSZZWY(config, ticketBought):
    ...
    #查询游客信息
    # userInfo = ''
    userInfo = dbObj.select("select user_id, name, mobile from t_user_identity_info where id = %d" % ticketBought['identity_info_id'])
    ...
    try:
        ...
        if 'true' == helper.subStr(responseBody, '<Result>', '</Result>'):
            sql = "update t_ticket_bought set out_app_code = 'YMXY', temp_receiving_code = 'None' where id = %d" % ticketBought['id']
