...
def ServiceYBX(config, mallBoughtList):
        ...
        for conf in config:
            try:
                if conf == 'mall_ProductID_' + str(mallBought['mall_product_id']) and config[conf] != '' :
                    # 给第三方增加门票
                    t = threading.Thread(target=addTicketToOuterYBX, args=(config, mallBought))
                    t.start()
                    ts.append(t)
                    break
...
# 同步门票到第三方系统的线程列表
def addTicketToOuterYBX(config, mallBought):
    # global dbObj
    ...
    userInfo = dbObj.select(
        "select user_id, name, mobile, id_number from t_user_identity_info where id = %d" % mallBought[
            'identity_info_id'])
    # userInfo = []
    # userInfo.append({
    # 'name': '微景通',
    ...
    orderInfo['effectdate'] = str(mallBought['plan_time'])[0:10]
    orderInfo['effectdate'] = orderInfo['effectdate'].replace('-', '', 3)
    orderInfo['otheruserid'] = int(mallBought['identity_info_id'])  # 其他用户ID号，指用户在第三方系统的用户账号，主要用于官方网站和官方微信的对接，第三方OTA不能使用。
    orderInfo['senderid'] = str(mallBought['order_detail_id'])  # 被接口方的业务单据ID，示例：同程网接口调用，此值表示同程网业务系统内对应的订单的单据号，此值在接口方系统内应为唯一值。不提供此参数时，系统将不会执行回调通知。提供这个参数还可以防止订单的重复提交。
    ...
    orderdetails = {}
    # orderdetails['productid'] = mallBought['id']
    orderdetails['productid'] = config['mall_ProductID_' + str(mallBought['mall_product_id'])]
    orderdetails['amount'] = mallBought['buy_count']
    ...
    payInfo = {}
    payInfo['orderid'] = str(mallBought['order_detail_id'])
    # payInfo['paypassword'] = config['payPassword']
    ...
    # print data;
    # exit();
    processOuterYBX(config, mallBought, postData, 1)
...
def processOuterYBX(config, mallBought, data, processCount):
    ...
    # print re;exit()
    # 记录请求结束时间
    # helper.getLog(helper.now(), 'addTicketToOuterYBX.recordEndTime.log')
    # 保存日志到数据库
    sql = "insert into t_order_outapp_msg (client_id, order_id, order_detail_id, type, outapp_code, content, create_time) \
           values ('%d', '%d', '%d', '%d', '%s', '%s', '%s') "
    values = (
        mallBought['client_id'], mallBought['order_id'], mallBought['order_detail_id'], 1, 'YBX', re,
        helper.now())
    # print sql
